...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Ejecuta el pipeline completo de ML, permitiendo especificar la ruta de salida del modelo."
    )
    # Define el argumento opcional --model-path_override
    parser.add_argument(
        "--model_path_override",
        type=str,
        default=None,
        help="Ruta de archivo completa para guardar el modelo entrenado. Si no se proporciona, usa 'models/best_model_pipeline.joblib'."
    )
    return parser.parse_args()

def main(model_path_override: str = None):
    ...
    # Lógica para determinar la RUTA FINAL del modelo
    ...
    try:
        ...
        with open(OUTPUT_DIR / "actual_metrics.json", "w") as f:
            json.dump(actual_metrics, f, indent=4)

        print(f"✔ Métricas del modelo ACTUAL guardadas en: {OUTPUT_DIR / 'actual_metrics.json'}")

        pipeline_loaded = load(MODEL_PATH)

        # detectar el modelo final
        if hasattr(pipeline_loaded, "named_steps") and "m" in pipeline_loaded.named_steps:
            final_model = pipeline_loaded.named_steps["m"]
        else:
            final_model = pipeline_loaded
        
        # solo parámetros serializables
        actual_params = {
            k: v for k, v in final_model.get_params().items()
            if isinstance(v, (int, float, str, bool)) or v is None
        }
        
        with open(OUTPUT_DIR / "actual_params.json", "w") as f:
            json.dump(actual_params, f, indent=4)
        
        print(f"✔ Parámetros del modelo ACTUAL guardados en: {OUTPUT_DIR / 'actual_params.json'}")
...
if __name__ == "__main__":
    args = parse_args()
    exit_code = main(model_path_override=args.model_path_override)
    sys.exit(exit_code)
